...
def test_match_to_list():
    ...
    assert match.to_list() == [{'field': 'IPV4_DST', 'value': IPv4Address('1.2.3.4'), 'mask': IPv4Address('255.255.255.0')}]

    match = Match(ipv6_dst='2000::1/64')
    assert match.to_list() == [{'field': 'IPV6_DST', 'value': IPv6Address('2000::1'), 'mask': IPv6Address('ffff:ffff:ffff:ffff::')}]
